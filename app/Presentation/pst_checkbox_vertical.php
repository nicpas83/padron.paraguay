...
class pst_checkbox_vertical extends Presentation {
    ...
    public function renderMaintForm() {
        ...
        $html.= '<div class="pull-left checkboxes col-sm-9 pl0 pr0">';
        foreach ($this->options as $key => $value) {
            $optionID = Inflector::camelize($id . $key);
            $optionName = $name . "[" . $key . "]";
            $html.= '<div class="mt5" ' . (isset($this->classparams['width']) ? 'style="width: ' . $this->classparams['width'] . '"' : '') . '>';
            ...
            $html.= '<label class="ml5" for="' . $optionID . '">' . $value . '</label>';
            $html.= '</div>';
        }
        $html.= '</div>';
        $html.= '</div>';
