...
class pst_dias extends Presentation {
    ...
    public function __construct($settings) {
        ...
        $this->options = ["Domingo", "Lunes", "Martes", "Miércoles", "Jueves", "Viernes", "Sábado"];
    }   
    ...
    public function renderMaintForm() {
        ...
        foreach ($this->options as $key => $value) {
            ...
            $optionName = $name . "[" . $key . "]";
            $html.= '<li class="mt7" ' . (isset($this->classparams['width']) ? 'style="width: ' . $this->classparams['width'] . '"' : '') . '>';
            $html.= '<input type="checkbox" ' . (in_array($key, $values) ? 'checked="checked"' : '') . ' name="' . $optionName . '" id="' . $optionID . '" value="' . $key . '" x-label="' . $value . '"/>';
            $html.= '<label class="checkbox ml5 mb2" for="' . $optionID . '">';
            $html.= '<h4 class="mt5"><span class="label label-default">' . $value . '</span></h4>';
            $html.= '</label>';
            $html.= '</li>';
        }
        $html.= '</ul>';
